...
static uint8 simpleProfileChar5Props = GATT_PROP_READ| GATT_PROP_WRITE_NO_RSP;

// Characteristic 5 Value
static uint8 simpleProfileChar5[SIMPLEPROFILE_CHAR5_LEN] = { 0, 0, 0, 0, 0,0 };
...
static uint8 simpleProfileChar7Props = GATT_PROP_READ | GATT_PROP_WRITE_NO_RSP | GATT_PROP_NOTIFY;
 ...
 * @return      None.
 */
 void battNotifyCB( linkDBItem_t *pLinkItem )
{
  ...
  //  if ( value & GATT_CLIENT_CFG_NOTIFY )
    {
      ...
      for(int i=0;i<noti.len;i++)
      {
        ...
        noti.value[i] =simpleProfileChar6[i] ;
