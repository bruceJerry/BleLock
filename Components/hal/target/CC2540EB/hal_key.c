...
#include "osal.h"
#include "hal_led.h"
extern uint8 dmaRdyIsr;
extern  uint8 Uart_Colsed;
...
#define HAL_KEY_DEBOUNCE_VALUE  15
...
#else
/* TOUCH_IRQ is at P1.0 */
#define TOUCH_IRQ_PORT   P1
#define TOUCH_IRQ_BIT    BV(0)
#define TOUCH_IRQ_SEL    P1SEL
#define TOUCH_IRQ_DIR    P1DIR

/* edge interrupt */
#define TOUCH_IRQ_EDGEBIT  BV(1)
#define TOUCH_IRQ_EDGE     HAL_KEY_FALLING_EDGE

/* TOUCH_IRQ interrupts */
#define TOUCH_IRQ_IEN      IEN2  /* CPU interrupt mask register */
#define TOUCH_IRQ_IENBIT   BV(4) /* Mask bit for all of Port_1 */
#define TOUCH_IRQ_ICTL     P1IEN /* Port Interrupt Control register */
#define TOUCH_IRQ_ICTLBIT  BV(0) /* P1IEN - P1.0 enable/disable bit */
#define TOUCH_IRQ_PXIFG    P1IFG /* Interrupt flag at source */

/*TOUCH_OUT is at P1.5 */
#define TOUCH_OUT_PORT   P1
#define TOUCH_OUT_BIT    BV(5)
#define TOUCH_OUT_SEL    P1SEL
#define TOUCH_OUT_DIR    P1DIR

/* edge interrupt */
#define TOUCH_OUT_EDGEBIT  BV(1)
#define TOUCH_OUT_EDGE     HAL_KEY_FALLING_EDGE

/* TOUCH_OUT interrupts */
#define TOUCH_OUT_IEN      IEN2  /* CPU interrupt mask register */
#define TOUCH_OUT_IENBIT   BV(4) /* Mask bit for all of Port_1 */
#define TOUCH_OUT_ICTL     P1IEN /* Port Interrupt Control register */
#define TOUCH_OUT_ICTLBIT  BV(5) /* P1IEN - P1.5 enable/disable bit */
#define TOUCH_OUT_PXIFG    P1IFG /* Interrupt flag at source */
...
#else
  TOUCH_OUT_SEL &= ~(TOUCH_OUT_BIT);    /* Set pin function to GPIO */
  TOUCH_OUT_DIR &= ~(TOUCH_OUT_BIT);    /* Set pin direction to Input */
  TOUCH_IRQ_SEL &= ~(TOUCH_IRQ_BIT);    /* Set pin function to GPIO */
  TOUCH_IRQ_DIR &= ~(TOUCH_IRQ_BIT);    /* Set pin direction to Input */
...
void HalKeyConfig (bool interruptEnable, halKeyCBack_t cback)
{
  ...
  if (Hal_KeyIntEnable)
  {
...
#else
    /* Rising/Falling edge configuratinn */
    PICTL |= 0x04;//(TOUCH_OUT_EDGEBIT);    /* Clear the edge bit */
    /* For falling edge, the bit must be set. */
  #if (TOUCH_OUT_EDGE == HAL_KEY_FALLING_EDGE)
    PICTL |= 0x06;
  #endif
     ...
     */
    TOUCH_OUT_ICTL |= TOUCH_OUT_ICTLBIT;
    TOUCH_OUT_IEN |= TOUCH_OUT_IENBIT;
    TOUCH_OUT_PXIFG = ~(TOUCH_OUT_BIT);


    
    /* Rising/Falling edge configuratinn */

    TOUCH_IRQ_ICTL &= ~(TOUCH_IRQ_EDGEBIT);    /* Clear the edge bit */
    /* For falling edge, the bit must be set. */
  #if (TOUCH_IRQ_EDGE == HAL_KEY_FALLING_EDGE)
    TOUCH_IRQ_ICTL |= TOUCH_IRQ_EDGEBIT;
  #endif
     ...
     * - Clear any pending interrupt
     */
    TOUCH_IRQ_ICTL |= TOUCH_IRQ_ICTLBIT;
    TOUCH_IRQ_IEN |= TOUCH_IRQ_IENBIT;
    TOUCH_IRQ_PXIFG = ~(TOUCH_IRQ_BIT);
...
#else
    TOUCH_OUT_ICTL &= ~(TOUCH_OUT_ICTLBIT); /* don't generate interrupt */
    TOUCH_OUT_IEN &= ~(TOUCH_OUT_IENBIT);   /* Clear interrupt enable bit */
    
    TOUCH_IRQ_ICTL &= ~(TOUCH_IRQ_ICTLBIT); /* don't generate interrupt */
    TOUCH_IRQ_IEN &= ~(TOUCH_IRQ_IENBIT);   /* Clear interrupt enable bit */
#endif  // !CC2540_MINIDK
...
uint8 HalKeyRead ( void )
{
  ...
  if ( !(TOUCH_OUT_PORT & TOUCH_OUT_BIT))    /* Key is active low */
 ...
 if (!( TOUCH_IRQ_PORT &  TOUCH_IRQ_BIT))    /* Key is active low */  
  {
    keys |=  TOUCH_IRQ;
  }
...
void halProcessKeyInterrupt (void)
{
  ...
  if((P1IEN&0x20))
  {
  if (TOUCH_OUT_PXIFG & TOUCH_OUT_BIT)  /* Interrupt Flag has been set */
  {
    TOUCH_OUT_PXIFG = ~(TOUCH_OUT_BIT); /* Clear Interrupt Flag */
    valid = TRUE;
  }
  }
if (TOUCH_IRQ_PXIFG & TOUCH_IRQ_BIT)  /* Interrupt Flag has been set */
  {
    TOUCH_IRQ_PXIFG = ~(TOUCH_IRQ_BIT); /* Clear Interrupt Flag */
    valid = TRUE;
  }
...
#else
  if (TOUCH_OUT_PXIFG & TOUCH_OUT_BIT)
#endif
  {
...
HAL_ISR_FUNCTION( halKeyPort1Isr, P1INT_VECTOR )
{
  ...
  if ((TOUCH_IRQ_PXIFG & TOUCH_IRQ_BIT)||(TOUCH_OUT_PXIFG & TOUCH_OUT_BIT))
  {
    halProcessKeyInterrupt();
  }
